...
did the trick for me on OSX. This is only a concern while in development, once you
...
create two connected apps: one for development using localhost and a second with
...
which enables running a development environment directly from the production URL. This
way you develop in a fully integrated mode and only have to setup a single Salesforce
connected app. Your Aerobatic OAuth callback URL is in the form: `https://your_app.aerobaticapp.com/auth/callback`.

After clicking Save, be sure to make note of your Consumer Key and Consumer Secret.

## Implementing OAuth
Now that we have an app, we need authentication wired up since that's a prerequisite for
being able to make Salesforce API calls. Salesforce supports three different
flavors which are described in [this article](https://www.salesforce.com/us/developer/docs/api_rest/Content/intro_understanding_authentication.htm) as follows:
...
defeating the point of OAuth which is to get out of the password business entirely. The
other two options are viable, both with their own pros and cons as we'll see.

### User-Agent Flow
This flow is implemented entirely in the browser, the web server does not get
involved at all. The user's browser loads a page (either full window or popup) pointing
to the URL:

```
https://login.salesforce.com/services/oauth2/authorize?redirectURI=https://yourappurl/oauthcallback.html
```
After the user logs in via the Salesforce owned page the window is
redirected to whatever was specified in the `redirectURI`. This URL must
be __HTTPS__ and needs to match the domain of the __Callback URL__ specified when the app was
setup in Salesforce.

On the `oauthcallback` page JavaScript is used to extract the `access_token` out of the query string and store
...
So it's not a good idea to store the token in a cookie. Stashing it in a JavaScript variable
in memory is more secure, but it means that a page refresh will necessarily require
re-authenticating since the token will be flushed from memory.
...
```
The `forcetk` object comes from the [Salesforce JavaScript REST Toolkit](https://github.com/developerforce/Force.com-JavaScript-REST-Toolkit),
a javascript library which is included in the page. The `setSessionToken` function
simply stashes the `access_token` in memory and includes it as an authorization
...
to OAuth allows keeping the consumer key securely stored on the server and avoids
...
works much the same as the user-agent flow, but the OAuth negotiation takes place
between Salesforce and the host web server rather than the user's browser.
The advantage to this approach is the `access_token` can be securely
stored on the server. Note the `access_token` would still need to be passed to the
browser if API calls were to be made directly from the browser, but as I'll cover
shortly, Salesforce doesn't allow this type of API invokation.
...
module that plugs into Passport to enable the web server flow for Salesforce.
The Aerobatic platform, which is implemented in Node and Express, makes use of
this module.  
...
Behind the scenes Aerobatic serves the correct html page based on whether or not
the user is in an authenticated state. The `login.html` page should include a relative link to the URL `/auth`
inviting the user to sign-in with Salesforce.
```html
...
configured provider including storing the `access_token` in session state in the
...
the `__config__.user` variable. The format of the object conform to the
[PassportJS normalized user profile schema](http://passportjs.org/guide/profile/).
...
browser must be routed through a proxy. This proxy could live on the origin host of
the app or it could be a CORS specific proxy on a separate domain. [James Ward](http://www.jamesward.com/) of Salesforce has written an [open-source sample CORS proxy](http://www.jamesward.com/2014/06/23/cross-origin-resource-sharing-cors-for-salesforce-com)
...
As discussed earlier, when using the Aerobatic OAuth integration,
the access token is not available to the browser. So in the `X-Authorization`
header we use a token in the form `@@user.accessToken@@`. Within the proxy
Aerobatic will substitute any token pattern with the expression nested between
the `@@` symbols.
...
I showed how the publishing system on www.aerobatic.io works. The blog post you are
reading right now was retrieved from GitHub and cached within the Aerobatic proxy.
